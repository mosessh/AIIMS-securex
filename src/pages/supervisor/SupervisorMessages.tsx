...
export default function SupervisorMessages() {
  ...
  const handleSend = () => {
    ...
    sendMessage.mutate(
      { recipientId, content: newMessage.trim(), messageType: "direct" },
      {
        onSuccess: () => {
          setNewMessage("");
  ...
  if (!selectedConversation && !showNewConversation) {
    return (
      <SupervisorMobileLayout title="Messages">
  ...
  if (showNewConversation) {
    return (
      <SupervisorMobileLayout title="New Message">
        <div className="space-y-4">
          <Button
            variant="ghost"
            size="sm"
  ...
  // Chat view
  return (
    <SupervisorMobileLayout title={selectedParticipant?.participantName || "Chat"}>
