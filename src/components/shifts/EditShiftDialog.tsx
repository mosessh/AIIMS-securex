...
export function EditShiftDialog({ shift, open, onOpenChange }: EditShiftDialogProps) {
  ...
  useEffect(() => {
    if (shift && open) {
      setValue("guardId", shift.guardId);
      setValue("siteId", shift.siteId);
      setValue("date", new Date(shift.startTime));
      setValue("startTime", format(new Date(shift.startTime), "HH:mm"));
      setValue("endTime", format(new Date(shift.endTime), "HH:mm"));
    }
  }, [shift, open, setValue]);
  ...
  const onSubmit = async (data: ShiftFormData) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to update shift. Please try again.";
      toast({
        title: "Error",
